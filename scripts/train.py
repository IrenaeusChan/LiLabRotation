...
class SupervisedDataset(Dataset):
    ...
    def __init__(self, 
        ...
        # I have already gone ahead and generated all the kmer.jsons
        # So for DNABERT we just need to load them in...
        if kmer != -1:
            kmer_path = data_path.replace(".csv", f"_{kmer}mer.json")
            if os.path.exists(kmer_path):
                logging.warning(f"Loading k-mer from {kmer_path}...")
                with open(kmer_path, "r") as f:
                    kmer = json.load(f)
